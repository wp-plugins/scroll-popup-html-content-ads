...
function scroll_popup_html_content_ads_filter($content)
{
	return 	preg_replace_callback('/\[SPHCA-POPUP:(.*?)\]/sim','scroll_popup_html_content_ads_filter_Callback',$content);
}

function scroll_popup_html_content_ads_filter_Callback($matches) 
{
	global $wpdb;
...
add_action('init', 'scroll_popup_html_content_ads_add_javascript_files');


add_filter('the_content', 'scroll_popup_html_content_ads_filter');
